...
Rectangle {
	...
	function save(){
		...
		Qt.inputMethod.hide();
		var eScheduleQueue=enguia.createEntity("EScheduleQueue");
		eScheduleQueue.userID=mShared.userID;
		eScheduleQueue.serviceID=serviceID;
		eScheduleQueue.comment=textArea.text;
		fillDates(eScheduleQueue)
		mSVC.metaInvoke(MSDefines.SPlaceScheduleQueue,"AddToQueue",function(ok){
		...
		},mShared.placeID, enguia.convertObjectToJson(eScheduleQueue));
	}
	Component.onCompleted: {
		var dt1=dt;
		mSVC.metaInvoke(MSDefines.SBlockSchedule,"GetNextTenDaysOpen",function(list){
		...
		},mShared.placeID,serviceID,enguia.convertToDateISOString(dt));
	}
}

...
Item{
    ...
    function append(title, value, id,img) {
        menuModel.append({"title":title,"value":value, "backColor":'#e7e3e7', "id":id,"img":img,"counter":0})
    }
    function remove(id){
    ...
    function setMsgCount(count){
        for(var i=0;i<menuModel.count;i++) {
			if(menuModel.get(i).title===enguia.tr("Messages")) {
                menuModel.get(i).counter=count;
                break;
            }
        }
    }
    function setNewsCount(count){
        for(var i=0;i<menuModel.count;i++) {
			if(menuModel.get(i).title===enguia.tr("News")) {
                menuModel.get(i).counter=count;
                break;
            }
        }
    }
    function setQualificationsCount(count){
        for(var i=0;i<menuModel.count;i++) {
			if(menuModel.get(i).title===enguia.tr("Qualifications")) {
                menuModel.get(i).counter=count;
                break;
            }
        }

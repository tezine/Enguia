...
Rectangle {
	...
	function generateReport(){
		...
		mSVC.metaInvoke(MSDefines.SReports,"GetTextOrderReport",function(list){
			loadingTextReport=false;
			for(var i=0;i<list.length;i++){
				var eTextReport=list[i];
				addToTable(eTextReport)
			}
		},mShared.placeID,comboProducts.getSelected(),btnStart.dt, btnEnd.dt,false,MSDefines.OrderTypeExternal)
	}

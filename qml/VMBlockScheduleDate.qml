...
Rectangle {
	...
	VMBlockScheduleTimeList{
		...
		onListItemClicked: {
			if(status===MSDefines.ScheduleMaxReached){
				mainStack.push({item:Qt.resolvedUrl("qrc:///BlockSchedule/VMBlockScheduleDateQueue.qml"),destroyOnPop:true, immediate:true, properties:{dt:calendar.selectedDate, serviceID:serviceID}})
				return;
			}
			if(!enguia.isValidTime(name))return ;//closed, contact, ...
			mainStack.push({item:Qt.resolvedUrl("qrc:///BlockSchedule/VMBlockScheduleConfirmation.qml"),destroyOnPop:true, immediate:true, properties:{dt:calendar.selectedDate, tm:name, serviceID:serviceID}})
		}
	}
	VMListEmptyRect{
		id: emptyRect
	...
	function calendarClicked(dt){
		...
		isLoading=true;
		var list= mSAgenda.getAvailableTime(mShared.placeID,serviceID, dt, function(list){
			isLoading=false;
	...
	function refreshMonth(dt){
		...
		mainIndicator.visible=true;
		mSAgenda.getAvailableAgenda(mShared.placeID, serviceID, dt,function(ok){

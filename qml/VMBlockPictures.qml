    ...
    Component.onCompleted: {
		...
		mSVC.metaInvoke(MSDefines.SPlacePictures,"GetPictures",function(list){
			for(var i=0;i<list.length;i++){
				...
				picsModel.append({"picID":ePlacePicture.picNumber, name:ePlacePicture.name, description:ePlacePicture.description, pictureURL:mSFiles.getPictureUrl(mShared.placeID,ePlacePicture.picNumber),
